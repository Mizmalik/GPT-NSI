...
for iteration in range(num_iterations):
    ...
    if iteration % eval_interval == 0 or iteration == num_iterations - 1:
        ...
        elapsed_time = end_iter_time - start_iter_time
        elapsed_hms = time.strftime("%H:%M:%S", time.gmtime(elapsed_time))
        print(f"Step {iteration}: perte d'entraînement {loss_values['train']:.4f}, perte de validation {loss_values['val']:.4f}, temps écoulé : {elapsed_hms}")
...
total_hms = time.strftime("%H:%M:%S", time.gmtime(total_time))
print(f"Temps total d'entraînement : {total_hms}")
